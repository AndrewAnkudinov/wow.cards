...
var View = function () {
	...
	createClass(View, [{
		...
		value: function setPreview(wError, src, width, height) {
			...
			// Выполнить после загрузки изображения в браузер
	...
	createClass(UploadingFile, [
		{
			...
			value: function loadChunk() {
				...
				function collectFD(file) {
					...
					fd.append('clip_id', vars.configPage.idDesign);
			...
			value: function collectFD() {
				...
				console.log('file: ', file);
				var fd = new FormData();  // Создать новую HTML-форму
				fd.append('clip_id', vars.configPage.idDesign);  // TODO: Зачем загрузчику файла знать о idDesign? По моему незачем.
				fd.append('name', file.name);
				fd.append('size', file.size);
				fd.append('sid', ses_id);
				fd.append('uident', ses_ident);
...
var Loader = {
	...
	addLis: function addLis() {
		...
		photoNVideo.els.btnRestore.onclick = function () {
			...
			jQuery.each(vars.configPage.oldOrder.files, function(index, element) {
				console.log(this);
				var serverFile = new File([], this.src, {type: 'image/png', size: this.size});  // size нельзя установить вручную, приходится передавать это значение отдельно
		...
		for (var j = 0; j < files.length; j++)
		{
			...
			if (f.size < 108664326) {
				...
				if (/*1 == 2 &&*/ files.uploadType != 'lib' && typeof URL.createObjectURL === 'function') {
					// Эмулировать загрузку файла для локальных файлов, загруженных браузер, а не на сервер (кроме IE)
	...
	checkLength         : function () {
		...
		// Вывести кол-во загруженные медиа-файлов
...
function sendClipData(Loader) {  // TODO: 2010-06-18 Нужны ли тут аргументы? Вроде нет
	...
	function collectData() {  // Собрать данные для отправки заказа
		...
		if (files === null) {
			...
			result.items = JSON.stringify(Loader.photoNVideo.items, function (key, value) {
	...
	*/
	...
	var idNewDesign = jQuery('#carouselSelectDesign .carousel-item.active').attr('data-id-design');  // TODO: бесплатный дизайн. А если полистали и передумали?
	//console.log(idNewDesign);
...
function init() {
	...
	function createEventListener() {
		...
		jQuery('#carouselSelectDesign').on('slide.bs.carousel', function () {  // TODO: запускать видео только у активного слайда
			// do something…
			console.log(this);
		});
