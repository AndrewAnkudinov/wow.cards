...
var vars =
	{
		...
		*/
		...
		btnCompleteUpload   : document.querySelector('#btnCompleteUpload'),
		btnChooseDesign     : document.querySelector('#btnChooseDesign'),
...
function showMinMax() {
	var mediaList = jQuery("#fotoNVideoContainer li");
...
jQuery(document).ready(function ($) {
	...
	var View = function () {
		...
		createClass(View, [{
			...
			value: function setPreview(wError, src, width, height) {
				...
				console.log(tempImg);
				tempImg.setAttribute('data-type', this.type);
				...
				if (jQuery(tempImg).attr('data-natural-width') == undefined) {  // Предотвратить повторное создание превью
					...
					if (this.type == 'image') {
						...
						var blobStub = createBlobStub(width, height);
	...
	var Loader = {
		...
		stopLoading         : function stopLoading(part) {
			...
			var img = document.querySelector('#fotoNVideoContainer img:not([data-crop-canvas])');
	...
	function sendClipData(Loader, texts) {
		...
		function sendData(data) {
			...
			*/
			...
			var idNewDesign = $('#carouselSelectDesign .carousel-item.active').attr('data-id-design');
			...
			var ajaxBody = {
				...
				less_photos: document.querySelector('#fotoNVideoContainer').getAttribute('data-less_photos'),
	...
	function setPageStyles() {  // 2021-05-17 TODO: Зачем это?
	...
	*/
	...
	$('#carouselSelectDesign').on('slide.bs.carousel', function () {
		// do something…
		console.log(this);
	});
