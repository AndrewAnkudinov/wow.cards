...
$dir_upload = PATH_TMP_USERFILES . '/' . $sesTmp['BX_USER_IDENT'];//.'/'.$_POST['clip_id'] . '/'
//создадим корневую папку юзера, если её нет
if (!is_dir($dir_upload)) {
	@mkdir($dir_upload, 0755);
}
//создадим папку проекта, если её нет
if (!is_dir($dir_upload . '/' . $_POST['clip_id'])) {
	@mkdir($dir_upload . '/' . $_POST['clip_id'], 0755);
}
$dir_upload = $dir_upload . '/' . $_POST['clip_id'];
$basename_file = $_POST['name'];
$f = fopen($dir_upload . '/' . $basename_file, "a");
fputs($f, file_get_contents($_FILES['chunk']['tmp_name']));
fclose($f);

// Закончить загрузку файла
// ... или продолжить её
if (filesize($dir_upload . '/' . $basename_file) == $_POST['size']) {
	...
	rename(
		$dir_upload . '/' . $basename_file,
		$dir_upload . '/' . $basename_new_file
	);
